...
def usage():
    ...
    print "      -I module ID: select a particular module for display"
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "b:c:m:M:I:s:H")
...
for i in range(t1041.GetEntriesFast()):
    ...
    for j in range(event.NPadeChan()):
        ...
        if i==0 and j==0: mapper=Mapper.Instance(pch.GetTimeStamp())
        ...
        if modID>=0:
            fiberID=mapper.ChannelID2FiberID(pch.GetChannelID())
            moduleID=fiberID/100
            if moduleID!=modID: continue
            print pch.GetChannelID()
        pch.GetHist(hw)
        nevents=nevents+1
        hw.Draw();
