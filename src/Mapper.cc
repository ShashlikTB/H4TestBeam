...
void Mapper::SetEpoch(unsigned long ts){  // redundant data here, clean this up
  ...
  else if (ts>=TBEvent::START_H4TB && ts<TBEvent::H4TB_MAP1 && FIBERMAP!=FIBERMAP_OCTOBER14){
    FIBERMAP=FIBERMAP_OCTOBER14;
    MakeMaps();
  }
  else if (ts>=TBEvent::H4TB_MAP1 && ts<TBEvent::H4TB_MAP2 && FIBERMAP!=FIBERMAP_OCTOBER2914){
    FIBERMAP=FIBERMAP_OCTOBER2914;
    MakeMaps();
  }
  else if (ts>=TBEvent::H4TB_MAP2 && ts<TBEvent::H4TB_MAP3 && FIBERMAP!=FIBERMAP_OCTOBER3014){
    FIBERMAP=FIBERMAP_OCTOBER3014;
    MakeMaps();
  }
  else if (ts>=TBEvent::H4TB_MAP3 && ts<TBEvent::H4TB_MAP4 && FIBERMAP!=FIBERMAP_OCTOBER3114){
    FIBERMAP=FIBERMAP_OCTOBER3114;
    MakeMaps();
  }
 else if (ts>=TBEvent::H4TB_MAP4 && ts<TBEvent::H4TB_MAP5 && FIBERMAP!=FIBERMAP_NOVEMBER0114){
    FIBERMAP=FIBERMAP_NOVEMBER0114;
    MakeMaps();
  }
 else if (ts>=TBEvent::H4TB_MAP5 && ts<TBEvent::H4TB_MAP6 && FIBERMAP!=FIBERMAP_MAY1815){
   FIBERMAP=FIBERMAP_MAY1815;
   MakeMaps();
 }
 else if (ts>=TBEvent::H4TB_MAP6 && ts<TBEvent::H4TB_MAP7 && FIBERMAP!=FIBERMAP_MAY2115){
   FIBERMAP=FIBERMAP_MAY2115;
   MakeMaps();
 }
 else if (ts>=TBEvent::H4TB_MAP7 && FIBERMAP!=FIBERMAP_JUN0916){
   FIBERMAP=FIBERMAP_JUN0916;
   MakeMaps();
 }
